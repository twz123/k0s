...
func CreateControllerUsers(spec *v1beta1.InstallSpec, homeDir string) error {
	users := getUserList(*spec.SystemUsers)
	var messages []string
	for _, v := range users {
...
// CreateControllerUsers accepts a cluster config, and cfgVars and creates controller users accordingly
func DeleteControllerUsers(spec *v1beta1.InstallSpec) error {
	cfgUsers := getUserList(*spec.SystemUsers)
	var messages []string
