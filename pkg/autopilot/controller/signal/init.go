...
// and worker modes.
func RegisterControllers(ctx context.Context, logger *logrus.Entry, mgr crman.Manager, delegate apdel.ControllerDelegate, k0sDataDir, clusterID string, leaseStatus leaderelection.Status) error {
	if err := k0s.RegisterControllers(ctx, logger, mgr, delegate, clusterID, leaseStatus); err != nil {
