...
func (s *APISuite) TestValidation() {
	...
	s.T().Run("invalid_api_address", func(t *testing.T) {
		...
		s.NotNil(errors)
		if s.Len(errors, 1) {
			s.ErrorContains(errors[0], `address: Invalid value: "something.that.is.not.valid//(())": not an IP address`)
		}
	})
	...
	s.T().Run("invalid_sans_address", func(t *testing.T) {
		...
		s.NotNil(errors)
		if s.Len(errors, 1) {
			s.ErrorContains(errors[0], `sans[0]: Invalid value: "something.that.is.not.valid//(())": neither an IP address nor a DNS name`)
		}
	})
}
