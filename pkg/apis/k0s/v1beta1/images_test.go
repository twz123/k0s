...
func TestStripDefaultsForDefaultImageList(t *testing.T) {
	for _, tt := range []struct {
		...
		{NllbTypeEnvoyProxy, func(t *testing.T, nllb *NodeLocalLoadBalancing) {
			...
			assert.Nil(t, nllb.TraefikProxy)
		}},
		{NllbTypeTraefikProxy, func(t *testing.T, nllb *NodeLocalLoadBalancing) {
			if assert.NotNil(t, nllb.TraefikProxy) {
				assert.Nil(t, nllb.TraefikProxy.Image)
			}
