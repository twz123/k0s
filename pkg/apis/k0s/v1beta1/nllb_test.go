...
func TestNodeLocalLoadBalancing_Unmarshal_ImageOverride(t *testing.T) {
	for _, tt := range []struct {
		...
		{ptr.To(NllbTypeTraefikProxy), func(t *testing.T, nllb *NodeLocalLoadBalancing) {
			require.NotNil(t, nllb.TraefikProxy)
			require.NotNil(t, nllb.TraefikProxy.Image)
			require.Contains(t, nllb.TraefikProxy.Image.Image, "example.com/")
...
func TestTraefikProxyImage_Unmarshal(t *testing.T) {
	yamlData := `
apiVersion: k0s.k0sproject.io/v1beta1
kind: ClusterConfig
metadata:
  name: TestTraefikProxyImage_Unmarshal
spec:
  network:
    nodeLocalLoadBalancing:
      type: TraefikProxy
...
`
	...
	for _, field := range []string{"image", "version"} {
		t.Run(field+"_empty", func(t *testing.T) {
			...
			require.NotNil(t, c.Spec.Network)
			require.NotNil(t, c.Spec.Network.NodeLocalLoadBalancing)
			require.NotNil(t, c.Spec.Network.NodeLocalLoadBalancing.TraefikProxy)
			assert.Equal(t, DefaultTraefikProxyImage(), c.Spec.Network.NodeLocalLoadBalancing.TraefikProxy.Image)
		})
	}
	...
	for _, test := range []struct {
		...
		{
			"image", `" "`,
			`network: nodeLocalLoadBalancing.traefikProxy.image.image: Invalid value: " ": must not have leading or trailing whitespace`,
		},
		{
			"version", `"*"`,
			`network: nodeLocalLoadBalancing.traefikProxy.image.version: Invalid value: "*": must match regular expression: ^[\w][\w.-]{0,127}(?:@[A-Za-z][A-Za-z0-9]*(?:[-_+.][A-Za-z][A-Za-z0-9]*)*[:][[:xdigit:]]{32,})?$`,
		},
	} {
		t.Run(test.field+"_invalid", func(t *testing.T) {
			c, err := ConfigFromBytes(fmt.Appendf(nil, yamlData, test.field, test.value))
