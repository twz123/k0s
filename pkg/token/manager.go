...
func RandomBootstrapSecret(role string, ttl time.Duration) (*corev1.Secret, *bootstraptokenv1.BootstrapTokenString, error) {
	token := bootstraptokenv1.BootstrapToken{
		...
		Usages: []string{
			// This usage is shared for all roles of the token which allows them
			// to execute calls to the k0s API. This is done because we need to
			// call the k0s API from windows workers during the join step.
			"api-auth",
		},
	}
