...
// Filter defines the func that tells the Debouncer if an item should be
// considered, or dropped without resetting the timeout.
type Filter[T any] func(item T) bool

// Callback defines the callback function when we trigger the debounce action
type Callback[T any] func(item T)
