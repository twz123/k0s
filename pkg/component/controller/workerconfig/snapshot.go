...
func makeConfigSnapshot(log logrus.FieldLogger, spec *v1beta1.ClusterSpec, konnectivityEnabled bool) (*configSnapshot, error) {
	...
	if spec.Network != nil &&
		...
		len(spec.ValidateNodeLocalLoadBalancer()) == 0 {
		...
		if dst.Type == v1beta1.NllbTypeEnvoyProxy {
			...
			if err := mergo.Merge(dst.EnvoyProxy, v1beta1.DefaultEnvoyProxy()); err != nil {
				return nil, err
			}

			if !konnectivityEnabled {
				dst.EnvoyProxy.KonnectivityServerBindPort = nil
			}
		}
