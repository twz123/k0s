...
func (a *apiConfigSource) Release(ctx context.Context) {
	...
	watch := watch.ClusterConfigs(a.configClient).
		...
		WithErrorCallback(func(err error) bool {
			log.WithError(err).Error("Failed to watch for updates to cluster configuration, starting over after backoff ...")
			return false
		})

	var lastKnownVersion string
	go func() {
		watch.Until(ctx, func(cfg *v1beta1.ClusterConfig) (bool, error) {
			// Push changes only when the config actually changes
			if lastKnownVersion != cfg.ResourceVersion {
				lastKnownVersion = cfg.ResourceVersion
				a.resultChan <- cfg
			}
			return false, nil
		})
