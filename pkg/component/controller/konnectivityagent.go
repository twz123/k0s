...
func (k *KonnectivityAgent) writeKonnectivityAgent(clusterConfig *v1beta1.ClusterConfig, serverCount uint) error {
	...
	if externalAddress := clusterConfig.Spec.Konnectivity.ExternalAddress; externalAddress != "" {
		...
		if nllb.IsEnabled() {
			...
			case v1beta1.NllbTypeTraefikProxy:
				k.log.Debug("Enabling node-local load balancing via ", nllb.Type)
				...
				if nllb.TraefikProxy.KonnectivityServerBindPort != nil {
					cfg.ProxyServerPort = uint16(*nllb.TraefikProxy.KonnectivityServerBindPort)
				} else {
					cfg.ProxyServerPort = uint16(*v1beta1.DefaultTraefikProxy().KonnectivityServerBindPort)
				}
			default:
