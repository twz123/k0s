...
{% set kubelet_ver = 'v' ~ kubelet_version ~ '+k0s' -%}
...
ubuntu   Ready    control-plane   4m41s   {{{ kubelet_ver | ljust(kubelet_ver_len)}}}   10.152.56.54   <none>        Ubuntu 22.04.1 LTS   5.15.0-1013-raspi   containerd://{{{ k0s_var('containerd_version') }}}
...
NAME          STATUS   ROLES    AGE    {{{ 'VERSION' | ljust(kubelet_ver_len) }}}   INTERNAL-IP    EXTERNAL-IP   OS-IMAGE             KERNEL-VERSION      CONTAINER-RUNTIME
node/ubuntu   Ready    <none>   5m1s   {{{ kubelet_ver   | ljust(kubelet_ver_len) }}}   10.152.56.54   <none>        Ubuntu 22.04.1 LTS   5.15.0-1013-raspi   containerd://{{{ k0s_var('containerd_version') }}}

{% set coredns = src_literal('CoreDNSImage') ~ ':' ~ src_literal('CoreDNSImageVersion') -%}
{% set metrics_server = src_literal('MetricsImage') ~ ':' ~ src_literal('MetricsImageVersion') -%}
