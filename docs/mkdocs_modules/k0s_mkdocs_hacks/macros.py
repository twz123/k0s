...
def define_env(env):
    ...
    src_literals = parse_src_string_literals()
    @env.macro
    def src_literal(name):
        return src_literals[name]

    k0s_vars = {}
    @env.macro
    def k0s_var(name):
        if name not in k0s_vars:
            proc = subprocess.run(['./vars.sh', name], capture_output=True, check=True, text=True)
            k0s_vars[name] = proc.stdout.strip()
        return k0s_vars[name]

    env.variables.k0s_version = os.environ['K0S_VERSION']
    env.variables.k0s_docker_version = os.environ['K0S_VERSION'].replace('+', '-')
    env.variables.k8s_version = 'v' + k0s_var('kubernetes_version')
