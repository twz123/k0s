...
# Ensures we have some semi-random machine-id.
ensure_machine_id() {
  case "${K0S_IN_DOCKER_ENSURE_MACHINE_ID-}" in
  0) return ;; # disabled
  1) ;;        # enabled
  *)           # auto detect
    [ ! -f /etc/machine-id ] || return ;;
  esac

  head -c16 /dev/urandom | hexdump -v -e '16/1 "%02x""\n"' >/etc/machine-id
  echo "$0: created /etc/machine-id" >&2
}
...
main() {
  ...
  *)
    ...
    : "${K0S_IN_DOCKER_ENABLE_CGROUPV2_NESTING:=0}"
    : "${K0S_IN_DOCKER_DNS_FIXUP:=0}"
    : "${K0S_IN_DOCKER_ENSURE_MACHINE_ID:=0}"
    ;;
  esac
  ...
  exec env \
    ...
    -u K0S_IN_DOCKER_ENSURE_MACHINE_ID \
    -u K0S_CONFIG \
    "$@"
}
