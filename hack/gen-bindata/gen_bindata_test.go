...
`
	...
	if goCode, err := os.ReadFile("bindata.go"); assert.NoError(t, err) {
		assert.Equal(t, expected, string(goCode))
	}

	require.NoError(t, os.Remove(filepath.Join("stripped", "bin", "first")))
	require.NoError(t, os.Remove(filepath.Join("stripped", "bin", "second")))
	require.NoError(t, os.Remove(filepath.Join("stripped", "bin")))
	require.NoError(t, os.Remove("stripped"))
	require.NoError(t, os.Remove("bindata"))
	require.NoError(t, os.Remove("bindata.go"))
