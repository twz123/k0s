...
func TestOnce_AwaitRacingWithSet(t *testing.T) {
	// This needs to run quite often to reliably catch any parallelism bugs on
	// constrained/oversubscribed hardware.
	for round := range 10_000 {
		synctest.Test(t, func(t *testing.T) {
			var underTest value.Once[int]
			...
			for num := range 4 {
				go func() {
					<-start
					actual := underTest.Await()
					assert.Equalf(t, 42, actual, "Goroutine %d in round %d", num+1, round+1)
				}()
			}
			...
			// Wait until all goroutines are parked and ready to be unleashed.
			synctest.Wait()

			// Unleash all parked goroutines at the same time.
			close(start)
