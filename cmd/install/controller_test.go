...
Flags:
  ...
  -c, --config string                                  config file, use '-' to read the config from stdin (default `+defaultConfigPath+`)
      ...
      --disable-components strings                     disable components (valid items: applier-manager,autopilot,control-api,coredns,csr-approver,endpoint-reconciler,helm,konnectivity-server,kube-controller-manager,kube-proxy,kube-scheduler,kubelet-serving-ca,metrics-server,network-provider,node-role,system-rbac,update-prober,windows-node)
