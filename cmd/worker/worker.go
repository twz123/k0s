...
func loadWorkerConfig(ctx context.Context, profile string, getter clientcmd.KubeconfigGetter) (workerconfig.Interface, error) {
	var workerConfig workerconfig.Interface
	...
	if err := retry.Do(
		func() error {
			...
			if err != nil {
				return fmt.Errorf("failed to load configuration for worker profile %q: %w", profile, err)
			}
