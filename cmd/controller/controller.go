	...
	if c.EnableDynamicConfig {
		configSource = lifecycle.GoFunc(&nodeComponents, func(ctx context.Context) (*lifecycle.Unit[clusterconfig.ConfigSource], error) {
			if applierManager != nil {
				...
				if err != nil {
					return nil, fmt.Errorf("failed to initialize api-config manifests saver: %w", err)
				}

				if err := controller.NewCRD(apiConfigSaver, "v1beta1", controller.WithCRDAssetsDir("k0s")).Unpack(); err != nil {
					return nil, err
				}
			}
			...
			if err != nil {
				return nil, fmt.Errorf("failed to create cluster config initializer: %w", err)
			}
